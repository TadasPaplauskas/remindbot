function Messenger(params)
{
    ...
    self.parseEvents = function(body, callback)
    {
        ...
        for (var entry of body.entry)
        {
            ...
            {
                // it's a postback
                if (message.postback !== unsigned)
                {
                    // use parser to route the payload
                    parser(message.postback.payload,
                        {
                    ...
                    continue; // no need to store that message
                }
    ...
    self.sendButtonsMessage = function(recipient, text, object, next)
    {
        var data = {
            attachment: {
                type: "template",
