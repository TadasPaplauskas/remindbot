function Messenger(params) {
    ...
    self.init = function (dependencies) {
        ...
        request({
            ...
            json: {
                ...
                greeting: { text: "Hi! Say \"hello\" or click \"Get started\" if you're new here." }
            }
        }, function (error, response, body) {
            if (error) throw error;
    ...
    self.sendButtonsMessage = function(recipient, text, buttons, next) {
        ...
        request({
            ...
            if (error) {
                ...
                // we cannot reach the user any more, no point in trying
                if (response.body.error.code == 200) {
                    console.log('User is not using remindbot anymore');
                    next();
                } else { // unknown case, gotta investigate
                    console.error('Error: ', response.body.error);
                }
            }
            else if (next) {
    ...
    self.sendTextMessage = function(recipient, text, next) {
        ...
        request({
            ...
            if (error) {
                ...
                // we cannot reach the user any more, no point in trying
                if (response.body.error.code == 200) {
                    console.log('User is not using remindbot anymore - cannot deliver');
                    next();
                } else { // unknown case, gotta investigate
                    console.error('Error: ', response.body.error);
                }
    ...
    self.requestClarification = function(reminder) {
        var reply = 'When exactly should I remind you to '+ reminder.text + '?';
